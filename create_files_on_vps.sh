#!/bin/bash
# Run these commands on your VPS (you're already connected)
...
class MailFilterBot:
    ...
    async def send_card_charged_notification(self, card_info, amount):
    ...
    async def setup(self):
        ...
        async def handle_new_message(event):
            ...
            if email_type == "otp":
                ...
                if card_info and amount:
                    await self.send_card_charged_notification(card_info, amount)
                else:
                    logger.warning(f"Could not extract card/amount info: card={card_info}, amount={amount}")
...
cat > /etc/systemd/system/mailbot.service << 'SERVICEEOF'
[Unit]
...
systemctl start mailbot.service

echo ""
echo "=== Setup Complete! ==="
echo "Bot is now running 24/7 as a system service"
echo ""
echo "Useful commands:"
echo "  Check status: systemctl status mailbot"
echo "  View logs: journalctl -u mailbot -f"
echo "  Restart bot: systemctl restart mailbot"
echo "  Stop bot: systemctl stop mailbot"
EOF
